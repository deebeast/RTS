 ...
 */
    private static double findMaxE(ArrayList<Task> al) {
        ...
        for (int i = 0; i < al.size(); i++) {
            if(al.get(i).E > maxE) maxE = al.get(i).E;
        }
        return maxE;
    }
 ...
 */
    static int gcdOf(Double P, Integer F) {
        ...
        if (a > b) return gcdOf((double)a-b, b); // a is greater
    ...
    /**
     * if there exists a path from source to destination in residual graph return 1
     * and store the path in updateSource matrix
     * @param residualGraph
     * @param source
     * @param destination
     * @param updateSource
        ...
        for (int i = 0; i < possFrames.size(); ) {
            for (int j = 0; j < al.size(); j++) {
                if( (2*possFrames.get(i)- gcdOf(al.get(j).P, possFrames.get(i))) <= al.get(j).P ) {
                    ...
                    System.out.println("Frame size " + possFrames.get(i) + " doesn't satisfy Third Condition " + maxE + " Hence removed");
                    possFrames.remove(i);
                    i = 0;
                }
            }
 ...
 */    
    ...
    static Double maxFlow(ArrayList<Task> al) {
        double maxFlow = 0;
        maxFlow = al.stream().map((curr) -> curr.E * (HyperPeriod / curr.P)).reduce(maxFlow, (accumulator, _item) -> accumulator + _item);
        return maxFlow;
    }
    ...
    public static long lcm_of_array_elements(int[] element_array) {
        ...
        while (true) {
            ...
            if (divisible) lcm_of_array_elements = lcm_of_array_elements * divisor;
            else divisor++; 
